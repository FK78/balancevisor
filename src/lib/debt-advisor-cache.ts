...
const PREFIX = "debt-advisor";

function makeKey(userId: string): string {
  return cacheKey(PREFIX, userId);
}

export function getCachedAdvice(userId: string): string | undefined {
  return getCached<string>(makeKey(userId));
}

export function setCachedAdvice(userId: string, markdown: string): void {
...
export function invalidateCachedAdvice(userId: string): void {
