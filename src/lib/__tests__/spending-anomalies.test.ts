...
describe("getSpendingAnomalies", () => {
  ...
  it("detects category with spending >50% above 3-month average", async () => {
    ...
    mockGetTrend.mockResolvedValue([
      ...
      { month: cm, category: "Food", color: "#f00", total: 300 },
      // History: Food at 100 each month -> avg 100
      { month: "2025-01", category: "Food", color: "#f00", total: 100 },
      { month: "2025-02", category: "Food", color: "#f00", total: 100 },
      { month: "2025-03", category: "Food", color: "#f00", total: 100 },
    ]);
  ...
  it("skips categories below 50% threshold", async () => {
    const cm = currentMonth();
    mockGetTrend.mockResolvedValue([
      { month: cm, category: "Food", color: "#f00", total: 140 },
      { month: "2025-01", category: "Food", color: "#f00", total: 100 },
      { month: "2025-02", category: "Food", color: "#f00", total: 100 },
      { month: "2025-03", category: "Food", color: "#f00", total: 100 },
    ]);
  ...
  it("skips categories with avg spend < 10", async () => {
    const cm = currentMonth();
    mockGetTrend.mockResolvedValue([
      { month: cm, category: "Misc", color: "#0f0", total: 50 },
      { month: "2025-01", category: "Misc", color: "#0f0", total: 5 },
      { month: "2025-02", category: "Misc", color: "#0f0", total: 5 },
    ]);
  ...
  it("skips new categories (no history)", async () => {
    const cm = currentMonth();
    mockGetTrend.mockResolvedValue([
      { month: cm, category: "New Cat", color: "#00f", total: 500 },
    ]);
  ...
  it("caps at 5 anomalies, sorted by pctAbove desc", async () => {
    ...
    const rows = [];
    for (let i = 1; i <= 7; i++) {
      rows.push({ month: cm, category: `Cat${i}`, color: "#000", total: 100 + i * 50 });
      rows.push({ month: "2025-01", category: `Cat${i}`, color: "#000", total: 50 });
      rows.push({ month: "2025-02", category: `Cat${i}`, color: "#000", total: 50 });
    }
